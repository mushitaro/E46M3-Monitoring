...
| tsunagi-m3 の `scripts/m-icons.mjs --word monitoring` | M ICON の 1024px 原版（repo の外） | `public/icons/monitoring{,-dev}{,-maskable}-*.png`（**コミットされる**。本番の組と、ワークス版が `brand-preview.mjs` で差し替える dev の組） |
