import { BufferedByteTransport, delay, now } from './bufferedByteTransport';
 ...
 *
 * The app this came from rejected WebUSB in its implementation notes, and that rejection is still
 * correct *for Windows*: Chromium claims USB devices there through WinUSB, an FTDI cable is bound to
 ...
 * FTDI_LATENCY_MS.
 *
 * ## Ported from the MSS54HP CSL Convert Tuner
 *
 * Changes, and only these: explicit WebUSB types instead of ambient globals; `Ds2Error` with a
 * machine-readable code instead of English prose (`FtdiLineError` deliberately excepted — see its
 * own note); `recoverRead` takes the settle time the link escalates; a thrown `transferOut` is
 * wrapped rather than left as a bare DOMException. Every comment block that records a MEASUREMENT
 * is carried across word for word, because several of them cannot be re-derived from the code.
 */
 ...
 *
 * The polarity here is a known trap. libftdi <= 1.4 exposed `SIO_RESET_PURGE_RX = 1`, but libftdi
 * 1.5 deprecated those functions and shipped `ftdi_tciflush()` — the *input* flush — using value 2,
 ...
 * So 8 | (2 << 8) = 0x0208.
 *
 * **8E1 is mandatory — do not "simplify" this to 8N1 while chasing line errors.** DS2 is
 ...
 * the Web Serial backend's `parity: 'even'` in DS2_SERIAL_DEFAULTS.
 */
 ...
 * enough that it dominates them. The per-exchange traces show the packetisation directly — 16 ms
 * steps in the ECHO delivery as well as the response tail, on frames that take 6 ms to transmit.
 *
 ...
 *  are refused rather than silently mis-clocked at a device that can write to an ECU. */
 ...
 *
 * A three-entry table rather than the general converter, because DS2 has exactly these three rates.
 * Three audited constants cannot be subtly wrong; a general converter can. 0x4138 and 0xC04E match
 * FTDI's published AN232B-05 table, which is the independent check that the derivation above is
 * right. All three rates are exact — zero baud error, including 125000.
 *
 * Only 9600 is ever selected by this app: `Ds2Control.REQUEST_BAUD_SWITCH` has no caller and a
 * diagnostics session never leaves the default rate. The other two are kept because deleting a
 * verified constant to save two lines is how the table stops being auditable against AN232B-05.
 */
 ...
 *
 * The source app had no test runner, which made this the only mechanism that could fail a wrong
 * constant anywhere. This one does have a test runner, and there IS a unit test for the table —
 * but the assertion stays, because the test and the assertion cover different places: the test
 * fails on a desk, and this fails in the browser of whoever is holding the phone, which is the only
 * environment where a wrong divisor becomes a garbled exchange with an ECU.
 */
 ...
 *
 * **Deliberately not a `Ds2Error`.** Everything else this file throws carries a code, because a UI
 * cannot branch on a sentence — but the value here is the opposite: the NAME has to match the
 * spelling Chromium's Web Serial implementation uses for the same physical event, so that the code
 * downstream sees one vocabulary from both backends. That consumer is concrete —
 * `BufferedByteTransport.readExact` puts this name into `detail.errorName`, and `simulator`'s
 * `breakLine()` produces 'BreakError' to exercise it. Wrapping it in a Ds2Error here would erase
 * the name at the only point it is read.
 */
...
export class WebUsbFtdiTransport extends BufferedByteTransport implements Ds2ByteTransport {
     ...
     *
     * **Deliberately not on `Ds2ByteTransport`.** Only this backend has a latency timer at all, and
     * `Ds2Link` must not know which backend it is holding; the hook that owns the run boundary
     * operates it instead.
     */
    ...
    private async sio(request: number, value: number, index = FTDI_PORT_INDEX): Promise<void> {
        ...
        if (!device) throw new Ds2Error('PORT_NOT_OPEN', 'USB device is not open');
        const result = await device.controlTransferOut({
            requestType: 'vendor', recipient: 'device', request, value, index,
        });
        if (result.status !== 'ok') {
            // WRITE_FAILED / 'electrical': every one of these runs inside open(), and every way it
            // fails — a cable pulled mid-configuration, a hub that cannot hold the port up — has the
            // same prescription as a failed data write. `ftdiRequest` is in detail so a failure
            // report can still say WHICH request, which the code alone cannot.
            throw new Ds2Error(
                'WRITE_FAILED',
                `FTDI control request 0x${request.toString(16)} failed (${result.status})`,
                { kind: 'electrical', detail: { ftdiRequest: request, status: result.status } },
            );
        }
    }
    ...
    private async setBaudRate(baudRate: number): Promise<void> {
        ...
        if (!divisor) {
            throw new Ds2Error(
                ...
                { kind: 'protocol', detail: { baudRate, supported: Object.keys(FTDI_DIVISORS).map(Number) } },
            );
        }
    ...
    async open(): Promise<void> {
        ...
        if (!usb) {
            throw new Ds2Error(
                'PORT_UNSUPPORTED',
                'WebUSB is not available in this browser (Chrome for Android required).',
                { kind: 'protocol' },
            );
        }
        ...
        // NotFoundError, and the caller must treat that name as a user cancel rather than a failure.
        ...
        // Clears the buffer AND the latch — the base class does both, so unlike the source app there
        // is no separate `pumpError = null` here.
        this.clearBuffer();
    ...
    private selectEndpoints(): void {
        ...
        throw new Ds2Error(
            'DEVICE_UNSUPPORTED',
            'No bulk endpoint pair found on this USB device — is it really an FTDI cable?',
            { kind: 'protocol' },
        );
    }
    ...
    private assertSupportedChipFamily(): void {
        ...
        if (!SUPPORTED_CHIP_VERSIONS.has(version)) {
            throw new Ds2Error(
                ...
                { kind: 'protocol', detail: { deviceVersionMajor: version } },
            );
        }
    }
     ...
     * offset 0 only would produce a plausible-looking response with two bytes of garbage every 64,
     * which is the worst failure this design can have: it does not announce itself.
     *
     ...
     * trick and they almost certainly complete in order — but "almost certainly" reorders a frame,
     * and the OE bit will say empirically whether a second is ever needed.
     */
    private startPump(): void {
        ...
        this.pumpExited = (async () => {
            ...
            while (this.pumpActive) {
                ...
                // through reading.
                //
                // `payload > 0` is also the base class's stated precondition on receive(), and this
                // is the backend that makes it matter: a status packet carrying no data is the
                // ordinary case here — 62.5 times a second at idle — and each one would otherwise
                // timestamp a byte arrival that never happened.
                if (payload > 0) this.receive(scratch.subarray(0, payload));
                if (fault !== null) {
                    this.latch(this.classifyLineStatus(fault));
                    return;
                }
            }
        })();
     ...
     *  readExact's message and its `detail.errorName` are both written against those spellings.
    ...
    async close(): Promise<void> {
        ...
        this.releaseWaiter();
        getUsb()?.removeEventListener('disconnect', this.onDisconnect);
        try {
            await this.pumpExited;
        } catch {}
        try {
     ...
     *
     * This is the structural difference the source app's implementation notes call out as forced and
     ...
     *
     * **Deliberately not on `Ds2ByteTransport`, and it has no caller in this app.** It stays a
     * concrete method on both backends because that asymmetry IS the finding: on Web Serial a baud
     * switch is the one moment that can desync the link, and on this one it costs nothing. A
     * capability with that difference inside it does not belong in a contract whose whole purpose is
     * to let the link stop caring which backend it holds.
     */
    ...
    async reopen(baudRate: number): Promise<void> {
        if (!this.device) throw new Ds2Error('PORT_NOT_OPEN', 'USB device is not open');
    ...
    async write(bytes: Uint8Array): Promise<void> {
        const device = this.device;
        if (!device) throw new Ds2Error('PORT_NOT_OPEN', 'USB device is not open');
        // Timed for the same reason as the Web Serial backend: the median should be ~0, and a
        // non-zero one would be a finding rather than a cost to accept.
        this.timing?.writeStart(now());
        let result;
        try {
            result = await device.transferOut(this.outEndpoint, bytes);
        } catch (e) {
            // The source app let a raw DOMException escape here. This package's contract is a coded
            // error — a UI cannot branch on a sentence — so it is wrapped, matching what the Web
            // Serial backend does with a rejected writer.
            throw new Ds2Error('WRITE_FAILED', `USB write failed: ${describeUsbError(e)}`, {
                kind: 'electrical',
                cause: e,
            });
        }
        ...
        if (result.status !== 'ok') {
            throw new Ds2Error('WRITE_FAILED', `USB write failed (${result.status})`, {
                kind: 'electrical',
     ...
     *
     * Stays synchronous by contract — the link's resync calls it without awaiting and
     * `drainUntilQuiet` reads `bufferedLength()` immediately afterwards. The driver flush is
     * therefore fire-and-forget. That is safe for the reason already documented on the base class's
     * purge(): it runs *between* exchanges, under the link's command gate, so there is no in-flight
     * frame for a late-landing flush to eat. If that invariant ever changes, this becomes a real
     * hazard rather than a tidy-up.
     */
    purge(): void {
        super.purge();
     ...
     * us. The settle default is 100 ms and identical to that backend's, because it is sized to the
     * break/idle condition on the wire, not to anything about the host API — and it is a parameter
     * for the same reason: `Ds2Link.drainUntilQuiet` lengthens it on each attempt, and a hardcoded
     * wait would put that number where the link's escalation cannot reach it.
     */
    async recoverRead(settleMs = 100): Promise<void> {
        ...
        // A break is recoverable; the device physically vanishing is not. Same split, and the same
        // wording, as the Web Serial backend's null `port.readable` check.
        if (this.deviceGone) {
            throw new Ds2Error(
                'READ_FAILED',
...
function describeUsbError(e: unknown): string {
