...
self.addEventListener('fetch', (event) => {
    ...
    //
    // The tuner answers EVERY navigation from /index.html, and says so — it has one route, so the
    // shortcut is exact there. Here it would not be: /usb-check is the page you open when the
    // phone itself is the thing under suspicion, and answering it with the main app would be the
    // worst possible silence — no error, no clue, on the one screen whose job is to produce clues.
    //
    // Next's static export writes a route as either `<path>/index.html` or `<path>.html` depending
    // on trailingSlash; both are tried rather than assumed, and scripts/verify-export.mjs asserts
    // that whichever one this build emitted is actually in ASSETS. The shell remains the last
    // resort, so a deep link to a route that does not exist still opens the app rather than the
