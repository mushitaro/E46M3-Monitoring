...
export function useAppUpdate(pollMs = 2 * 60 * 1000) {
    ...
    useEffect(() => {
        listenForPrecache();
        // Every setState in check() sits behind `await fetch`, so none of them runs during this
        // effect and the cascade the rule guards against cannot happen. The rule cannot see
        // through the async boundary. Deferring the first check through a timer to satisfy it
        // would add a timer to clean up and delay the one check that matters most: the one on
        // mount, when the app has just been opened and a stale build is most likely.
