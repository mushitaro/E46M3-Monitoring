 ...
 *
 * So it is used in exactly one direction: false says why a network-dependent control is inert; true
 * only lets the attempt happen. Whether it actually worked is the request's answer, reported on the
 * control, never predicted here. Treating `true` as a promise would produce the worst version of
 * this — a control that claims something is possible and then fails with no explanation.
 *
 * This app makes exactly one network request of its own after load: the update check in
 * useAppUpdate. Everything else it does is a cable and a cache. That is a narrow use, and it is the
 * reason this hook reports rather than decides.
 *
 ...
 * (Same shape as useInstallPrompt.)
 */
