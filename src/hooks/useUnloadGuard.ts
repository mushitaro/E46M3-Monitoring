 ...
 * during the seconds when that keystroke can leave an actuator energised with no page left running
 * to send its stop.
 *
 ...
 * removes the commonest way a phone interrupts itself, and `useHiddenWitness` records it when
 * something does. This hook stays exactly as strong as the platform allows and no stronger.
 *
 ...
 *
 * That makes the error case free rather than special: every operation in useDs2Link returns the link
 * to `connected` on both its success and its catch path, so a failure drops `active` and the guard
 * goes with it.
 *
 ...
 *                 operation: a job run is seconds, but a datalog is a drive and can run for hours,
