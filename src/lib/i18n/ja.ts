...
export const ja: Localised = {
    ...
    disclaimer_preview: {
        ...
        sessions: '故障コードの読み出し結果、データログ、通信ログ、ECU の識別情報（VIN を含む）',
        sessionsWhen: 'SYNC を押して保存したときに送ります。',
        recordsTitle: 'エラーの記録',
        records: '失敗したジョブの ECU・ジョブ名・エラーの文面と、その前後の通信記録',
        recordsWhen: 'ジョブが失敗したときに自動で送ります。通信できないときは端末に残し、次に送ります。',
    ...
    sessions_errors_note:
        '読み取りや記録が失敗したとき、このワークス版は ECU・ジョブ・エラー文・直前の通信ログを自動で送ります。送れなかった分はこの端末で待ち、次に送れたときに送ります。',
