...
`.prg` のハッシュ）も持たせるとしている。`SgbdDump/Program.cs` は既に両方を書くが、
**手元の 63 ダンプは 0 件しか持っていない**——古い exe で取ったものだから。無いものを
ここで作れば、それは我々がでっち上げた値になる。全数を取り直したときに入る
（`python tools/dump_modules.py`）。それまでは「どのダンプから出たか」は言えるが、
「そのダンプがいつ、どの .prg から出たか」は言えない。
"""
