...
EWS3: dict[str, tuple[str, str]] = {
    ...
    # EWS3 のみ。"SK in das EWS4.3 Steuergeraet schreiben"、MODE に LOCK_SERVER_SK がある。
    'STEUERN_EWS4_SK': (
        'イモビライザの秘密鍵(SecretKey、16 バイト)を書き込みます。引数 MODE には '
        '`WRITE_SERVER_SK`（書込）のほかに **`LOCK_SERVER_SK`（ロック）**があり、'
        '**ロックした鍵を戻すジョブは SGBD にありません。** 現在値を読み出す手段も'
        'ありません。ディーラーの鍵データ無しに触る領域ではありません。',
        'This writes the immobiliser secret key — 16 bytes. Besides `WRITE_SERVER_SK` the MODE '
        'argument offers **`LOCK_SERVER_SK`**, and **the SGBD has no job that unlocks it again.** '
        'There is no way to read the current value first either. This is not territory to enter '
        'without the dealer\'s key data.',
    ),
    'STEUERN_DIGITAL': (
